...
# Define the script you want to copy
script_to_copy="$script_name"
...
# Check if the script_to_copy exists in the current directory
if [ ! -f "${script_to_copy}" ]; then
    echo "File ${script_to_copy} does not exist in the current directory."
    exit 1
fi

# Define the directories you want to copy the script into in the required order
dirs=("targetdriver" "targetio" "targetcalib")

# Check if all directories exist
for dir in "${dirs[@]}"; do
    if [ ! -d "${dir}" ]; then
        echo "Directory ${dir} does not exist in the current directory."
        exit 1
    fi
done

# If all checks passed, copy the script into each directory and run it
for dir in "${dirs[@]}"; do
    cp "${script_to_copy}" "${dir}"
    echo "Copied ${script_to_copy} to ${dir}"
    (cd "${dir}" && bash "${script_to_copy}")
    echo "Ran ${script_to_copy} in ${dir}"
done

# Define the order and list of scripts
order=(0 1 2)  # The order is now based on the order of dirs
script_list=("${dirs[@]/%//${script_to_copy}}")

echo "Now executing the brycesTargetBuilder.sh scripts"
...
echo "Finished running all scripts. Now exiting, Goodbye!"
